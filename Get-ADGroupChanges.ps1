...
if ($UserName) {
        ...
        if (!(([adsisearcher]"(samaccountname=$global:Username)").FindOne()))
            {
                Write-Warning "[x] Account $global:Username Not found in AD. please ensure you've typed it correctly and try again."
                $ANRlookup = ([adsisearcher]"(anr=shamilov)").FindAll().Properties.samaccountname;
                if ($ANRlookup)
                    {
                        Write-Host "Potential similar looking accounts:" -ForegroundColor Cyan;
                        $ANRlookup;
                    }
                break;
            }
